...
# def EM_cluster(img, k, error=10e-4, iter_n=10):
#     likelihood_arr = []
#     means_arr = []
#     means, cov, pis = initialization(img, k)
#     likelihood = 0
...
#         means_arr.append(means)
#     likelihood_arr = np.array(likelihood_arr)
#     print('Completed {} iterations'.format(iter_n))
#     return labels, means, cov, pis, likelihood_arr, means_arr

# def plot_cluster_img(filename, k):
#     iter_n = 10
#     small_img, orig_img = read_img(filename=filename, size=(1.0, 1.0))  # Use full size
#     x, y, z = small_img.shape  # Use unblurred dimensions
#     # Save original image (unblurred)
#     plt.figure()
#     plt.axis("off")
#     plt.imshow(small_img)
#     plt.title('Original Image')
...
#     try:
#         img = flatten_img(small_img)  # Cluster on unblurred image
#         labels, means, cov, pis, likelihood_arr, means_arr = EM_cluster(img, int(k), error=0.001, iter_n=iter_n)
#         em_img = recover_img(means[labels], X=x, Y=y, Z=z)
    ...
    try:
        ...
        for i in range(len(cov)):
            if np.linalg.det(cov[i]) <= 0:
                cov[i] += np.eye(cov[i].shape[0]) * 1e-6
    except Exception as ex:
        print(f"Initialization error: {ex}")
        # Fallback
        means = np.random.rand(k, img.shape[1])
        cov = np.array([np.eye(img.shape[1]) for _ in range(k)])
        pis = np.ones(k) / k
    return means, cov, pis
...
def EM_cluster(img, k, error = 10e-4, iter_n = 10):
    ...
    while (abs(likelihood - new_likelihood) > error) and (cnt < iter_n):
        ...
        means_arr.append(means)
    likelihood_arr = np.array(likelihood_arr)
    print('Completed {} iterations'.format(cnt))
    return labels, means, cov, pis, likelihood_arr, means_arr
...
def plot_cluster_img(filename,k):
    # Visualize demo images
    # filename='img//L7.jpg'
    iter_n=5  # Reduce iterations for faster processing
    small_img, orig_img = read_img(filename = filename, size = (0.5, 0.5))  # Downsample for speed
    x, y, z = small_img.shape  # Use unblurred dimensions
    # Store img
    plt.figure()
    plt.axis("off")
    ...
    try:
        img = flatten_img(small_img)  # Cluster on unblurred image
        labels, means, cov, pis, likelihood_arr, means_arr = EM_cluster(img, int(k),error = 0.01, iter_n=iter_n)  # Looser error tolerance
        em_img = recover_img(means[labels], X=x, Y=y, Z=z)
        plt.figure()
        plt.axis("off")
        plt.imshow(em_img)
        ...
        clustering_results = {
            'num_clusters': int(k),
            'iterations': len(likelihood_arr),
            'final_likelihood': float(likelihood_arr[-1]) if likelihood_arr.size > 0 else None,
            'converged': len(likelihood_arr) < iter_n
        }
        print(f"Clustering completed: {clustering_results}")

    except Exception as ex:
        print(f"Error during clustering: {ex}")
        clustering_results = {'error': str(ex)}
